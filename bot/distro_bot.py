...
BOT_TOKEN = "8747731698:AAGO23w9VpRfjQfaLG07790mHPgm1VfMzb4"
...
ADMIN_IDS = {133154291}  # Eugene
BETA_IDS = {133154291, 111000637}  # Eugene, Marina
...
WELCOME_TEXT = """🔐 <b>CheburMail</b> — мессенджер, который не заблокируют

Работает поверх Яндекс Почты и Mail.ru — российских серверов из белых списков.
Сквозное шифрование: провайдер видит только метаданные, но не содержимое.

В отличие от Signal (заблокирован) и Telegram (нестабилен) — почта работает всегда."""

ABOUT_TEXT = """🔐 <b>CheburMail</b>

E2E мессенджер поверх Яндекс Почты / Mail.ru.

<b>Почему не Signal / Telegram?</b>
• Signal заблокирован в РФ, нужен VPN
• Telegram нестабилен, серверы за рубежом
• Яндекс и Mail.ru — в белых списках, их не заблокируют
• Почта переживёт «чебурнет» — это внутренняя инфраструктура

<b>Шифрование:</b> X25519 + XSalsa20-Poly1305 (libsodium)
<b>Транспорт:</b> IMAP/SMTP (обычная почта)

<b>Возможности:</b>
• Текст, фото, файлы, голосовые — всё E2E зашифровано
• Обмен ключами через QR-код
• PIN-код / биометрия на вход
• Автоочистка IMAP
• Проверка обновлений
• Без трекинга, аналитики и рекламы

<b>Код открыт:</b> github.com/shizzka/cheburmail"""
