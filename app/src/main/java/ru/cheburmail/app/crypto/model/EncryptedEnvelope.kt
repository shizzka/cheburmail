...
data class EncryptedEnvelope(
    ...
    companion object {
        ...
        fun fromBytes(data: ByteArray): EncryptedEnvelope {
            // crypto_box(empty_plaintext) даёт ciphertext длины ровно MAC_BYTES,
            // поэтому wire payload = nonce + MAC валиден. Раньше require был
            // строгим > и отвергал такие сообщения.
            require(data.size >= CryptoConstants.NONCE_BYTES + CryptoConstants.MAC_BYTES) {
                "Data too short: need at least ${CryptoConstants.NONCE_BYTES + CryptoConstants.MAC_BYTES} bytes, got ${data.size}"
            }
