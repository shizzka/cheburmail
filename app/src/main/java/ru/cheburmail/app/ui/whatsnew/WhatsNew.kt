...
object WhatsNew {
    ...
    val ENTRIES: List<WhatsNewEntry> = listOf(
        ...
        WhatsNewEntry(
            ...
            changes = listOf(
                "Приложение блокируется при сворачивании в фон — после возврата снова нужен PIN",
                "Удаление сообщений по сети теперь авторизовано: только автор может удалить своё",
                "Алиасы поддерживаются в групповых командах: если админ ответил с другого своего аккаунта — это распознаётся правильно",
                "PIN-код переехал на PBKDF2 + соль + throttling. Старый PIN мигрируется автоматически",
                "ImapClient не виснет навсегда: зависший fetch принудительно прерывается через 90 сек",
                "Безопасное сохранение файлов от контактов (защита от вредоносных имён)"
            )
        ),
        WhatsNewEntry(
            versionCode = 18,
